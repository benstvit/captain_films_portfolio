<template>
  <section>
    <div class="flex justify-between items-start my-4 p-4 bg-gray-100">
      <div
        v-for="(partner, index) in partners"
        class="flex justify-around items-start"
        :key="partner.name">
          <nuxt-img
            v-if="!partner.selected || menuDisplay"
            :src="partner.imgUrl"
            :alt="partner.name"
            @click="setActivePartner(index)"
            class="h-[20vh] w-fit object-contain hover:cursor-pointer hover:opacity-80 hover:opacity-80 my-4 mx-8"/>
          <Transition name="fade">
            <div v-if="partner.selected" class="h-[25vh] max-w-[30vh] my-4 mx-8">
              <span><a class="text-center text-teal-600 font-captainbold hover:text-teal-600 hover:text-lg" :href="partner.site" target="_blank">{{partner.name}}</a></span>
              <p class="mb-4 font-captainlight text-md justify whitespace-normal">{{ partner.description}}</p>
            </div>
          </Transition>
      </div>
    </div>
  </section>
</template>

<script>
export default {
  name: 'MyPartners',
  data() {
    return {
      partners: [
        {
          name: 'Mori Film Lab',
          imgUrl: 'https://cdn.shopify.com/s/files/1/0406/4313/3595/files/Screen_Shot_2020-09-23_at_5.02.01_pm_1070x.png?v=1614355014',
          description: 'was created by two young passionates, Cole and Raph. More than a shop, they created a community and offer the holy trinity of film developing: quality, speed and affordability.',
          selected: false,
          site: 'https://morifilmlab.com/'
        },
        {
          name: 'L\'Atelier KZG',
          imgUrl: '/KZG.png',
          description: 'was created by photographs for photographs and offers a mix of craftsmanship and technology with pigment printing on a wide variety of archival papers',
          selected: false,
          site: 'http://www.kzg.be/'
        },
        {
          name: 'Desenio',
          imgUrl: 'data:image/png;base64,',
          description: 'Mix of craftsmanship and technology, l\'Atelier KZG was created by photographs for photographs and offers pigment printing on a wide variety of archival papers',
          selected: false,
          site: 'https://desenio.be/fr-be/cadres-photo/'
        },
      ]
    }
  },
  computed: {
    menuDisplay() {
      const findActive = this.partners.filter(e => e.selected === true);
      return findActive.length === 0;
    },

  },
  methods: {
    setActivePartner(index) {
      this.partners.map(partner => partner.selected = false);
      const setActive = this.partners.map((e, i) => {
        if (i === index && e.selected) this.partners.forEach(p => p.selected = false);

        return i === index ? {...e, selected: true} : {...e };
      });
      this.partners = setActive;
    }
  }
}
</script>
